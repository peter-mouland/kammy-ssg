...
// NEW: Remove last draft pick(s)
export async function removeLastDraftPick(formData: FormData) {
    const divisionId = formData.get("divisionId")?.toString();
    if (!divisionId) {
        throw new Error("Missing division ID");
    }
    ...
    try {
        ...
        if (existingPicks.length === 0) {
            throw new Error("No picks to remove");
        }

        // Always remove exactly 1 pick (the most recent one)
        const actualRemoveCount = 1

        console.log(`🗑️ Removing last pick from division ${divisionId}`);

        // Use the efficient deleteLastDraftPicks function to remove 1 pick
        const { deleteLastDraftPicks } = await import('./sheets/draft');
        const removedPicks = await deleteLastDraftPicks(divisionId, 1);

        // Calculate new draft state after removals
        const remainingPicksCount = existingPicks.length - 1;

        // Recalculate draft state
        const newPickNumber = remainingPicksCount + 1;
        const totalPicks = draftOrder.length * (draftState.picksPerTeam || 15);

        // Determine whose turn it is now
        const pickIndex = (newPickNumber - 1) % draftOrder.length;
        const roundNumber = Math.ceil(newPickNumber / draftOrder.length);
        const isSnakeRound = roundNumber % 2 === 0;
        const userIndex = isSnakeRound ? draftOrder.length - 1 - pickIndex : pickIndex;
        const currentUser = draftOrder[userIndex];

        const newDraftState = {
            ...draftState,
            currentPick: newPickNumber,
            currentUserId: currentUser?.userId || draftState.currentUserId,
            isActive: newPickNumber <= totalPicks
        };
        ...
        // Sync to Firebase
        try {
            await FirebaseDraftSync.updateDraftState(divisionId, {
                currentPick: newDraftState.currentPick,
                currentUserId: newDraftState.currentUserId,
                isActive: newDraftState.isActive,
            ...
            await FirebaseDraftSync.addDraftEvent(divisionId, {
                type: 'pick-removed',
                data: {
                    removedPicks: removedPicks.map(p => ({
            ...
            console.log(`🔥 Pick removal synced to Firebase: 1 picks removed`);
        } catch (error) {
            console.error("🔥 Failed to sync pick removal to Firebase:", error);
        }

        return {
            success: true,
            action: 'removeLastPick',
            removedPicks: removedPicks,
            newState: newDraftState
        };
    } catch (error) {
...
// Helper function to remove pick from sheets (you'll need to implement this based on your sheets setup)
async function removeDraftPickFromSheets(pickNumber: number, divisionId: string) {
    // This depends on your Google Sheets implementation
    // You might need to:
    // 1. Find the row with this pick
    // 2. Delete the row or clear the data
    // Example implementation:

    try {
        // Import your sheets functions
        const { deleteDraftPick } = await import('./sheets/draft');
        await deleteDraftPick(pickNumber, divisionId);
        console.log(`🗑️ Removed pick ${pickNumber} from sheets`);
    } catch (error) {
        console.error(`❌ Failed to remove pick ${pickNumber} from sheets:`, error);
        throw error;
    }
}
