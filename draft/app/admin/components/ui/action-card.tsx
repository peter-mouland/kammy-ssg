...
export const ActionCard = ({
    ...
    return (
        <div className={`${styles.actionCard} ${recommended ? styles.recommended : ''}`}>
            ...
            {hasSuccess && fetcher.data?.message && <div className={styles.successMessage}>{fetcher.data.message}</div>}

            {hasError && <div className={styles.errorMessage}>{fetcher.data.error}</div>}
        </div>
    );
};
