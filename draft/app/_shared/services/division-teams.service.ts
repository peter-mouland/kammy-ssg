...
export async function getDivisionTeamsDocument(
    ...
    try {
        ...
        throw new Error(`Failed to get division teams for ${divisionId} GW${gameweek}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
...
export async function getUserTeamForGameweek(
    ...
    try {
        ...
        throw new Error(`Failed to get user team for ${userId} in ${divisionId} GW${gameweek}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
...
export async function getUserTeamHistory(
    ...
    try {
        ...
        throw new Error(`Failed to get team history for ${userId} in ${divisionId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
 ...
 */
export async function getAllTeamsInDivision(
    divisionId: string,
    gameweek: number
): Promise<UserTeamRoster[]> {
    try {
        const divisionDoc = await getDivisionTeamsDocument(divisionId, gameweek);
        if (!divisionDoc) {
            return [];
        ...
        throw new Error(`Failed to get all teams for ${divisionId} GW${gameweek}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
 ...
 */
export async function divisionDocumentExists(
    divisionId: string,
    gameweek: number
): Promise<boolean> {
    try {
...
export async function getAvailableGameweeks(divisionId: string): Promise<number[]> {
    try {
        ...
        throw new Error(`Failed to get available gameweeks for ${divisionId}: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
 ...
 */
export async function createDivisionTeamsDocument(
    document: DivisionTeamsDocument
): Promise<void> {
    try {
        const db = getFirestoreInstance();
        ...
        throw new Error(`Failed to create division teams document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
...
export async function updateDivisionTeamsDocument(
    ...
    try {
        ...
        throw new Error(`Failed to update division teams document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
 ...
 */
export async function deleteDivisionTeamsDocument(
    divisionId: string,
    gameweek: number
): Promise<void> {
    try {
        ...
        throw new Error(`Failed to delete division teams document: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
}
