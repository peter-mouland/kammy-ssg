...
describe('teamCountLimit', () => {
    ...
    it('passes when playerIn brings the team count to exactly 2', () => {
        ...
        const alreadyInRoster = makePlayer({ id: 99, code: 299, web_name: 'SameTeam', position: 'mid', team_code: PLAYER_FREE_CB.team_code });
        ...
        const result = teamCountLimit(makeContext(transfer, { divisionRosters: makeDivisionRosters(roster), fplPlayersByCode }));
        expect(result.passed).toBe(true);
    });
    ...
    it('blocks when playerIn would result in 3 players from the same real-world team', () => {
        ...
        const sameTeamPlayer = makePlayer({ id: 30, code: 300, web_name: 'SameTeam', position: 'fb', team_code: PLAYER_CB1.team_code });
        const roster = makeStandardRoster();
        ...
        const fplPlayersByCode = {
            ...makeContext(makeTransfer({ transferType: 'TRANSFER', playerIn: PLAYER_FREE_CB, playerOut: PLAYER_MID1 })).fplPlayersByCode,
            [sameTeamPlayer.code]: sameTeamPlayer,
        };

        // Now transfer in another player from team_code 11 (CB1's team)
        const anotherSameTeam = makePlayer({ id: 31, code: 301, web_name: 'AnotherSameTeam', position: 'mid', team_code: PLAYER_CB1.team_code });
    ...
    it('uses warning severity (not blocking) for LOAN_END team count violations', () => {
        ...
        const rosters = makeDivisionRosters(roster);
        const playerIn = makePlayer({ id: 31, code: 301, web_name: 'LoanBack', position: 'mid', team_code: PLAYER_CB1.team_code });
        const fplPlayersByCode = {
            ...makeContext(makeTransfer({ transferType: 'LOAN_END', playerIn, playerOut: PLAYER_MID1 })).fplPlayersByCode,
            [sameTeamPlayer.code]: sameTeamPlayer,
