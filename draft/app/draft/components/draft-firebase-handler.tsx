...
export const DraftFirebaseHandler: React.FC<DraftFirebaseHandlerProps> = ({
    ...
    const handleDraftEvent = useCallback((event: DraftEvent & { id: string }) => {
        ...
        switch (event.type) {
            ...
            case 'draft-synced':
                ...
                showToast({
                    message: `📋 Draft synced from Google Sheets! ${syncData?.picksCount || 0} picks, ${syncData?.isActive ? 'active' : 'completed'}`,
                    type: 'info',
                    duration: 5000
                });
            ...
            case 'draft-reset':
                ...
                showToast({
                    message: `⚠️ Draft completely reset from Google Sheets! All Firebase data cleared and rebuilt. ${resetData?.picksCount || 0} picks restored.`,
                    type: 'warning',
                    duration: 8000
                });
    ...
    }, [currentUserId, showToast]); // Dependencies for stable callback
