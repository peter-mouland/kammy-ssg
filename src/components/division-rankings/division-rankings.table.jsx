...
const PlaceHolder = ({ manager }) => (
    <tr className="row">
        <td className="cell cell--manager">
            <div className="ph--manager">{manager}</div>
        </td>
        {positions.map((position) => (
            <Fragment key={position.label}>
                <td className={`cell cell--${position.key}`}>
                    <div className="ph--position-rank" />
                </td>
                <td className={`cell cell--pair cell--${position.key}`}>
                    <div className="ph--position-score" />
                </td>
            </Fragment>
        ))}
        <td className="cell cell--total">
            <div className="ph--total-rank" />
        </td>
        <td className="cell cell--pair cell--total">
            <div className="ph--total-score" />
        </td>
    </tr>
);
...
const DivisionRankingsTable = ({ points, type, handleRowHover, managers, rank }) => (
            ...
            {points.length > 0 ? (
                ...
                <Fragment>
                    {managers.map((managerName) => (
                        <PlaceHolder key={managerName} manager={managerName} />
                    ))}
                </Fragment>
            )}
        </tbody>
