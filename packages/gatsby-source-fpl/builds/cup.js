...
module.exports = ({ googleCupData, createNodeId }) =>
    googleCupData.map((cup) => {
        ...
        if (gatsbyData.player1Name) {
            gatsbyData.player1___NODE = createNodeId(`fpl-players-${gatsbyData.player1Name}`);
        }
        if (gatsbyData.player2Name) {
            gatsbyData.player2___NODE = createNodeId(`fpl-players-${gatsbyData.player2Name}`);
        }
        if (gatsbyData.player3Name) {
            gatsbyData.player3___NODE = createNodeId(`fpl-players-${gatsbyData.player3Name}`);
        }
        if (gatsbyData.player4Name) {
            gatsbyData.player4___NODE = createNodeId(`fpl-players-${gatsbyData.player4Name}`);
        }
