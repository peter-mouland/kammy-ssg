...
| 2026-07-29 | **[Will slow down future work]** | **`googleapis` is the wrong dependency for this app.** `import { google } from 'googleapis'` pulls every Google API — 1.7s — and the app uses exactly one, Sheets. Google publishes `@googleapis/sheets`. **Measured: `googleapis` 1675ms.** For the scoped comparison the only `@googleapis/*` package installed here is `sqladmin` (a transitive dep), which imports in **231ms** — so treat ~7x as the indicative saving and confirm against `@googleapis/sheets` itself once installed. It matters per-worker in tests and, far more, on every Cloud Function **cold start**.

**The swap is smaller than it first looks.** Checked, not assumed:
- `google.auth.JWT` is **identity-equal** to `google-auth-library`'s `JWT` (`google.auth.JWT === JWT` is `true`), and `google-auth-library` is **already a direct dependency** of `draft` (`^9.15.1`). So the auth half is `import { JWT } from 'google-auth-library'` with no new package and no behaviour change.
- The two non-obvious settings — the custom `transporter` (`fetchTransporterRequest`) and `useJWTAccessWithScope = true`, which avoids a token-endpoint round trip — are `google-auth-library` features, not `googleapis` ones, so they carry over untouched.
- That leaves one real change: `google.sheets({ version: 'v4', auth })` → `sheets({ version: 'v4', auth })` from `@googleapis/sheets`, in [common.ts](../draft/app/_shared/lib/sheets/utils/common.ts).

It is still worth its own change because it is the app's only Sheets path — but it is covered by the six MSW sheets tests, which run the real client, the real auth handshake and the real parsing, so a break would surface immediately. | [common.ts](../draft/app/_shared/lib/sheets/utils/common.ts) |
