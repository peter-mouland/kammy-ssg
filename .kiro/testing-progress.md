...
| **G24** | The sheet has a **fourth division, `greatScott`** (Great Scott League, 8 managers) across `Divisions`, `UserTeams`, `DraftOrder` and `DraftState` — 32 managers, not 24. `DivisionId` is a three-way union, so the app ignores it. Supporting it is a feature: scoring, standings and promotion/relegation all assume a fixed set. `/admin` now warns rather than crashing | — |
