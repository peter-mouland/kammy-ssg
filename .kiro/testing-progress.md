...
| id | task | Part |
|---|---|---|
| ~~**G23**~~ | ~~Services that flatten a cause into a string~~ — **done**: 13 sites across `teams`, `transfers`, `scoring` and `admin` did `new Error('Failed to X: ' + e.message)`, discarding the cause. All now pass `{ cause: error }`, and the `: ${…'Unknown error'}` suffix is gone — it was a lie once the cause was preserved, and doubly so when the cause was an `AppError` (a plain object, so the ternary always chose "Unknown error") | — |
| ~~**G24**~~ | ~~The sheet has a **fourth division, `greatScott`**~~ — **done**: supported, and the rules are now read from the sheet's new `promotion`/`relegation`/`cup` columns rather than inferred from the id. Outstanding: greatScott has no `division-teams` documents until points processing runs for it. Originally read: the sheet has a fourth division (Great Scott League, 8 managers) across `Divisions`, `UserTeams`, `DraftOrder` and `DraftState` — 32 managers, not 24. `DivisionId` is a three-way union, so the app ignores it. Supporting it is a feature: scoring, standings and promotion/relegation all assume a fixed set. `/admin` now warns rather than crashing | — |
